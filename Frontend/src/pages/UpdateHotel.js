...
export default function UpdateHotel(props){
    ...
    function sendData(e){
     ...
     axios.put(`http://localhost:8070/Hotel/update/${sessionStorage.editHotel_id}`,editHotel).then(()=>{
     

        let editHotel_id=sessionStorage.editHotel_id;
        console.log(editHotel_id);
...
useEffect(() =>{
    const id = props.id;
    let editHotel_id=sessionStorage.editHotel_id;
    axios.get(`http://localhost:8070/Hotel/get/${editHotel_id}`).then((res) => {
        console.log(res);
        let data=res.data.addhotels;
